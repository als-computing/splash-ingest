...
def test_ingest_service_init_readers():
    ...
    assert len(reader_modules) == 1, 'duplicate specs in reader modules, only one should be loaded'
    reader_module = reader_modules['reader']
    assert reader_module.ingest(None, 'test_reader_1') == 'test_reader_1', 'reader1:ingest successfully called'
