...
def test_create_job(client: TestClient):
    ...
    response: CreateJobResponse = client.post(url="/api/v1/ingest_jobs", data=request.json(), headers={API_KEY_NAME: key})
    assert response.status_code == 200
    job_id = response.json()['job_id']

    response = client.get(url="/api/v1/ingest_jobs/" + job_id)
    assert response.status_code == 403, 'ingest jobs wihtout api key'

    response = client.get(url="/api/v1/ingest_jobs/" + job_id + "?" + API_KEY_NAME + "=" + key)
